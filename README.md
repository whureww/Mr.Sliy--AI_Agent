...
- 修复：AI 修复出现「全删零增」diff——优化输出上限固定 2000 tokens，文件稍大即被截断、JSON 解析失败后空结果仍按成功返回。现按代码规模动态放大输出上限（最高 8000），空结果显式失败（后端双层兜底 + 前端拦截，不再进入对比页）
- 修复：主工作区「新建」按钮未跟随中英文切换（硬编码文案改走 i18n）
